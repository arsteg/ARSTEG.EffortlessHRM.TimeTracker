...
namespace TimeTrackerX.AppUsedTracker
{
    public class ActiveApplicationInfomationCollector
    {
        ...
        public void ApplicationFocusStart(string appName, string appTitle)
        {
            ...
            {
                _focusedApplication.Add(
                    appName,
                    new FocushedApplicationDetails
                    {
                        AppTitle = appTitle,
                        Duration = 0,
                        TotalKeysPressed = 0,
        ...
        public void ApplicationFocusEnd(
            string appName,
            int TotalMouseClick,
            int TotalKeysPressed,
            int TotalMouseScrolls,
        ...
        )
        {
            if (_focusedApplication.ContainsKey(appName))
            {
                ...
                _focusedApplication[appName].Duration = Convert.ToDouble(
                    _focusedApplication[appName].Duration + elapsed.TotalMilliseconds
                );
